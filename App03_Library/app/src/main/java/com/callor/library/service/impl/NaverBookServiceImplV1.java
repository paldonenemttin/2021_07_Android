...
public class NaverBookServiceImplV1 implements NaverBookService {
    ...
    public String getNaverBooks(String search) {
         ...
         *    도반된 method(getBOok() )를 호출하면서
         *    필요한 매개변수를 전달하면
         *    Retrogit은 naver server에 요청을 하고
         *    그 요청을 "비동기 방식"으로 기다리게 한다
         * 2. 비동기 방식
         ...
         * Retrofit이 응답을 받아서 처리할 ecent(call back)
         *
         * Call Back Event 핸들링 코드
         * Callback interface를 익명 클래스로 선언하는 클래스
         * Skeletone code
         */
        retrofitReturn.enqueue(new Callback<NaverParent>() {
            ...
            public void onResponse(Call<NaverParent> call, Response<NaverParent> response) {
                ...
                if (resCode < 300) {
                    ...
                    // Naver에서 수신한 전체 데이터에서 도서 List 정보만 추출하기
                    ...
                    // MainActicity에서 전달받은 recyclerView에
                    // Adapter를 setting
